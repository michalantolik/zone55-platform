...
public static partial class MarkdownTextRenderer
{
    public static string ToHtml(string? markdown)
    {
        ...
        {
            var trimmed = rawLine.TrimEnd().Trim();
            ...
            if (TryRenderHeading(html, paragraph, ref inUnorderedList, ref inOrderedList, trimmed))
            {
                continue;
            }
            ...
            if (trimmed.StartsWith("> ", StringComparison.Ordinal))
            {
                FlushParagraph(html, paragraph);
                CloseLists(html, ref inUnorderedList, ref inOrderedList);
                html.Append("<blockquote>").Append(RenderInline(trimmed[2..])).Append("</blockquote>");
                continue;
            }
            ...
            if (orderedMatch.Success)
            {
                ...
                html.Append("<li>").Append(RenderInline(orderedMatch.Groups["text"].Value)).Append("</li>");
                continue;
            }
    ...
    private static bool TryRenderHeading(
        ...
        string value)
    {
        var heading = value.StartsWith("### ", StringComparison.Ordinal) ? (3, value[4..])
            : value.StartsWith("## ", StringComparison.Ordinal) ? (2, value[3..])
            : value.StartsWith("# ", StringComparison.Ordinal) ? (2, value[2..])
            : (0, string.Empty);
        ...
        FlushParagraph(html, paragraph);
        CloseLists(html, ref inUnorderedList, ref inOrderedList);
        html.Append("<h").Append(heading.Item1).Append('>')
            .Append(RenderInline(heading.Item2))
            .Append("</h").Append(heading.Item1).Append('>');
        return true;
    }
    ...
    private static void FlushParagraph(StringBuilder html, List<string> paragraph)
    {
        ...
        html.Append("<p>").Append(RenderInline(string.Join("<br />", paragraph))).Append("</p>");
        paragraph.Clear();
    }
