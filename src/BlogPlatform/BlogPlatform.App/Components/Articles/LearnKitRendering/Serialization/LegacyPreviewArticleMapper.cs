...
public static class LegacyPreviewArticleMapper
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    public static LearnKitArticleDetails ToLearnKitArticle(PostDetails article)
    {
        var blocks = ParseBlocks(article.BodyHtml)
            .Select((block, index) => ToLearnKitBlock(block, index + 1))
            .ToArray();

        return new LearnKitArticleDetails
        {
            Id = $"preview-{article.Slug}",
            Slug = article.Slug,
            Title = article.Title,
